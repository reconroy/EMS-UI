...
const Department = () => {
  ...
  const departments = [
    { id: 1, name: 'Information Technology', description: 'IT and Systems', status: 'Active' },
    { id: 2, name: 'Human Resources', description: 'HR Management', status: 'Active' },
    { id: 3, name: 'Finance', description: 'Financial Operations', status: 'Inactive' },
  ];
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log({ departmentName, description, status: isActive ? 'Active' : 'Inactive' });
    setDepartmentName('');
    setDescription('');
    setIsActive(true);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <h1 className={`text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r ${theme === 'dark' ? 'from-purple-400 to-purple-600' : 'from-blue-400 to-blue-600'}`}>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="departmentName" className={`block mb-2 ${textClass}`}>
          ...
          <table className="w-full">
            <thead>
              <tr className={`border-b ${theme === 'dark' ? 'border-purple-500/20' : 'border-blue-200'}`}>
                <th className={`px-6 py-4 text-left ${textClass}`}>Department Name</th>
            ...
            <tbody>
              {departments.map((department) => (
                ...
                >
                  <td className={`px-6 py-4 ${textClass}`}>{department.name}</td>
                  <td className={`px-6 py-4 ${subTextClass}`}>{department.description}</td>
